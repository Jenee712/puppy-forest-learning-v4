...
import { getDailyWordTrainWords, getWordTrainBank, type WordTrainCard as WordCard } from "@/data/wordTrainBank";
...
export default function WordTrainGame({ grade, day, coins, onReward }: { grade: string; day: number; coins: number; onReward: (amount: number) => void }) {
  const words = useMemo(() => getDailyWordTrainWords(grade, day), [grade, day]);
  const bankSize = useMemo(() => getWordTrainBank(grade).length, [grade]);
  ...
  const speakClue = async (card: WordCard) => {
    const run = speechRun.current + 1; speechRun.current = run; setSpeakingId(`${card.id}-clue`);
    const cleanMeaning = card.meaning.replace(/[，。！？；：、“”‘’…·—]/g, " ").replace(/\s+/g, " ").trim();
    try { await playTts(cleanMeaning, { language: "zh", segment: "word", playbackRate: 1 }); }
    catch { /* 临时语音错误不打断游戏 */ }
    finally { if (speechRun.current === run) setSpeakingId(null); }
  };
  const onDrop = (event: DragEvent<HTMLDivElement>) => {
    ...
    if (card) {
      answer(card);
      if (card.id === target.id) void speakBilingual(card);
    }
  };
  ...
  return (
    <section className="page-surface word-train-page">
      <header className="word-train-header">
        <div><span className="section-kicker">森林游戏站 · {grade} · 第{day}天</span><h1>单词小火车</h1><p>每日轮换8个词，答题、发音和间隔复习一起完成。</p></div>
        <div className="word-train-wallet"><span>🪙</span><strong>{coins}</strong><small>学习金币</small></div>
      </header>

      <div className="word-train-progress"><span>今日第 {Math.min(round + 1, words.length)} / {words.length} 站</span><div><i style={{ width: `${status === "finished" ? 100 : round / words.length * 100}%` }} /></div><strong>本级词库 {bankSize} 词 · 新词 +1 🪙</strong></div>
      ...
      {status !== "finished" ? (
        <div className="word-train-challenge">
          <div className="word-clue"><span>{target.icon}</span><div><small>找到这个单词</small><strong>{target.meaning}</strong><button className={speakingId === `${target.id}-clue` ? "speaking" : ""} onClick={() => void speakClue(target)} type="button" aria-label={`听中文提示：${target.meaning}`}>{speakingId === `${target.id}-clue` ? "■ 停止" : "🔊 听中文提示"}</button></div></div>
